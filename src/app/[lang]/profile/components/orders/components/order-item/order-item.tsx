...
const OrderItem = ({ route, type, price, date, images } : {route: string, type: string, price: number, date: string, images: Array<any>}) => {
  return (
    <Link className={'flex flex-col w-full gap-8 p-4 border border-control-100 rounded-xl'} href={route}>
      <Div className={'w-fill justify-between items-center'}>
        <Div className={'items-center gap-4'}>
          ...
          <Div className={'flex-col'}>
            <Text typography={['sm', 'sm']} type={'normal'}>{type}</Text>
            <Text color={'grey.700'} typography={['xs', 'xs']} type={'medium'}>{date}</Text>
          </Div>
        </Div>
        <Div className={'items-center gap-4'}>
          <Text typography={['sm', 'sm']} type={'normal'}>${price}</Text>
          <ArrowRightIcon />
        </Div>
      </Div>

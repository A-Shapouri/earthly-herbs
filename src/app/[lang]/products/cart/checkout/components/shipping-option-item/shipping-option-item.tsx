...
const ShippingOptionItem = ({option, selectable = false, inList = false}: ShippingOptionItemProps) => {
  ...
  const {shippingOption} = useSelector((state: RootState) => state.shop);
  ...
  return (
    <Div className={classNames('rounded-3xl justify-between border p-4 w-full gap-2 items-center',
      ...
      <Div className={'flex-col gap-2'}>
        <Text type={'normal'} color={'black'} typography={['md', 'md']}>{option.title}</Text>
        <Text type={'normal'} color={'grey.700'} typography={['base', 'base']}>{option.description}</Text>
      </Div>

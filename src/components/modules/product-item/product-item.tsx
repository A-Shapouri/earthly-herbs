...
const ProductItem = ({image, title, price, rate, isHealthy, isNew, id}: ProductItemProps) => {
  ...
  return (
    <Div
      ...
      <Link href={getParseRoute({pathname: 'en/shop/[id]', query: {id: id}})} className={'flex absolute top-2 w-full justify-between'}>
      ...
      <Link href={getParseRoute({pathname: 'en/shop/[id]', query: {id: id}})} className={'flex rounded-t-xl md:rounded-t-2xl'}>
      ...
      <Div
        ...
        <Link href={getParseRoute({pathname: 'en/shop/[id]', query: {id: id}})} className={'flex flex-col justify-between items-start gap-1  h-full'}>

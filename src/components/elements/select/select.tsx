          ...
          {label ? (
            <span
              ...
              className={classNames(
                ...
                disabled ? '!cursor-not-allowed text-control' : (error ? 'text-danger' : (show ? `${COLORS[color]}` : 'text-control'))
              )}
            >
              {label}
            </span>
          ...
          {autoComplete ? (
            ...
            selectedText !== '' && (
              <Div className={'grow flex select-none !leading-none'}>
                <Text className={'!leading-none'} typography={['sm', 'sm']} align={'start'} color={disabled ? 'grey.500' : 'grey.800'}>
                  {selectedText}
                </Text>
              </Div>
            )
          ...
          {searchedItems && searchedItems.length > 0 && typeof searchedItems === 'object' ? searchedItems.map((option: any) => (
            ...
            >
              <Text
                ...
                align={'start'}
                type={`${option[text] === selectedText ? 'normal' : 'medium'}`}
                typography={['sm', 'sm']}
                data-name={option[id]}
                key={option[id]}

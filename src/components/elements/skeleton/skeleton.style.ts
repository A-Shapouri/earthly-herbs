...
export const SHAPES = ({ size, shape }: { size: Size, shape: Shape }) => {
  ...
  if (size === 'tiny') {
    className = `min-w-4 p-0 ${className}`;
  } else if (size === 'small') {
    className = `min-w-8 md:!min-w-10 p-0 ${className}`;
  } else if (size === 'medium') {
    className = `min-w-10 md:!min-w-12 p-0 ${className}`;
  } else if (size === 'large') {
    className = `min-w-12 md:!min-w-14 p-0 ${className}`;
  } else if (size === 'huge') {
    className = `min-w-14 md:!min-w-16 p-0 ${className}`;
  }
  return className;
};
